 ...
 * Phase 56 — Gmail OAuth callback.
 * Exchanges code for tokens, stores encrypted in connected_accounts.
 * Redirects to /settings?tab=connected on success.
 */

import { NextRequest, NextResponse } from 'next/server';
import { neon } from '@neondatabase/serverless';
import { exchangeGmailCode, encryptToken } from '@/lib/email-oauth';
...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    const msg = error ?? 'OAuth cancelled';
    return NextResponse.redirect(`${redirectBase}/settings?tab=connected&error=${encodeURIComponent(msg)}`);
  }
  ...
  await sql`
    INSERT INTO connected_accounts (
      ...
      ${userId}::uuid, 'gmail', ${tokens.email}, ${tokens.displayName},
  ...
  return NextResponse.redirect(`${redirectBase}/settings?tab=connected&success=gmail`);
}
